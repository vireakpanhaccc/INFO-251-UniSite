...
>>>>>>> Stashed changes
      ...
      <div class="card-content px-4">
        ...
        <p>${major.description.split(/(?<=\.)\s+/)[0] || ''}</p>

        <a class="absolute bottom-4 right-4 py-1.5 px-2 bg-[#2c3e50ec] text-sm text-white rounded hover:bg-[#2c3e50] active:brightness-80" href="#/majors/${major.id ?? major._id}" target="_blank">${t('visit_details')}</a>
      </div>
    </div>
  `).join('');
}
