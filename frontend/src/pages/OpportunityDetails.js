  ...
  el.innerHTML = `
    <div class="university-page flex flex-col gap-4">
      ...
      <section class="px-4 md:px-8 lg:px-16">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div class="col-span-2">
            ...
            <p class="mb-2">${scholarship.description}</p>
          </div>
          <div class="col-span-2 md:col-span-1">
            <p class="mb-2"><strong>Provider:</strong> ${scholarship.provider}</p>
            <p class="mb-2"><strong>Amount:</strong> ${scholarship.amount}</p>
            <p class="mb-2"><strong>Deadline:</strong> ${scholarship.deadline}</p>
            <p class="mb-2"><strong>Application Link:</strong> <a href="${scholarship.link}" target="_blank" class="text-blue-600 underline">${scholarship.link}</a></p>
          </div>
          <div class="col-span-2 md:col-span-1">
            <p class="mb-2"><strong>Eligibility:</strong> ${scholarship.eligibility}</p>
            <p class="mb-2"><strong>Available at Universities:</strong> ${relatedUniversities.map(uni => uni.name).join(', ')}</p>
          </div>
        </div>
      </section>
          ...
          ${relatedUniversities.map(uni => `
            <div class="bg-white rounded-lg p-4 shadow-md hover:shadow-lg transition-shadow">
              <div class="flex items-center space-x-4 mb-3">
                ...
                <div>
                  <h3 class="font-semibold text-lg">${uni.name}</h3>
                  <p class="text-gray-600">${uni.location.city}</p>
                </div>
              </div>
              <p class="text-sm text-gray-700 mb-3">${uni.type} • Established ${uni.established}</p>
              <a href="#/universities/${uni.id}" class="inline-block px-4 py-2 bg-[#2c3e50ec] text-white rounded hover:bg-[#2c3e50] transition-colors">
                View University
              </a>
            </div>
          `).join('')}
      ...
      <section class="px-4 md:px-8 lg:px-16 pt-6 pb-10 bg-white">
        <h2 class="text-center text-3xl font-semibold mb-4">Related Scholarships</h2>
          ...
          ${relatedScholarships.slice(0, 3).map(relatedScholarship => `
            <div class="uni-card h-[500px] border border-gray-300 shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300 relative max-w-[600px]">
              ...
              <div class="card-content px-4">
                <h3 class="text-md font-bold">${relatedScholarship.name || 'No Name Provided'}</h3>
                <p>Provider: ${relatedScholarship.provider || 'Unknown'}</p>
                <p>Amount: ${relatedScholarship.amount || 'N/A'}</p>
                <p>Deadline: ${relatedScholarship.deadline || 'N/A'}</p>
                <a class="absolute bottom-4 right-4 py-1.5 px-2 bg-[#2c3e50ec] text-sm text-white rounded hover:bg-[#2c3e50] active:brightness-80" href="#/opportunities/${relatedScholarship.id}">Visit Details</a>
              </div>
            </div>
          `).join('')}
        </div>
        ...
        <div class="bg-white rounded-lg p-6 max-w-4xl mx-auto">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <p class="text-gray-700 mb-4">${scholarship.eligibility}</p>
              <h3 class="text-xl font-semibold mb-3">Application Deadline</h3>
              <p class="text-gray-700 text-lg font-medium">${scholarship.deadline}</p>
            </div>
            <div>
              ...
              <a href="${scholarship.link}" target="_blank" class="inline-block px-6 py-3 bg-[#2c3e50ec] text-white rounded-lg hover:bg-[#2c3e50] transition-colors font-semibold">
                Apply Now
              </a>
            </div>
          </div>
